...
def split_dataset(input_dir: Path, output_dir: Path, split_ratio: float = 0.8):
    ...
    # We infer the source image from the filename, generally everything before '_gt', '_crop', or just use the prefix.
    # Ex: '0cc55cc1-s8_gt0040_fiber.png' -> '0cc55cc1-s8'
    groups = defaultdict(list)
    for img_name, ann in annotations.items():
        # Check if 'source_image' explicitly exists in annotation
        if 'source_image' in ann:
            ...
            if '_gt' in img_name:
                ...
                # If we cannot split safely, treat each image individually
                src = img_name
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", required=True, help="Input directory containing images/, masks/, annotations.json")
    parser.add_argument("--output", required=True, help="Output directory where train/ and val/ will be created")
    ...
    split_dataset(in_path, out_path, args.ratio)
