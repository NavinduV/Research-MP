...
Usage:
    ...
    python src/train/train_effnet.py --mode train --crops-dir data/crops_gt_sam --epochs 50

    # Train on crops from YOLO ground-truth (class folders only)
    python src/train/train_effnet.py --mode train --crops-dir data/crops_gt --epochs 50

    # Evaluate on validation set
    python src/train/train_effnet.py --mode val --crops-dir data/crops_gt_sam

    # Classify a single crop image
    python src/train/train_effnet.py --mode predict --image path/to/crop.png
"""
...
class CropClassificationDataset(Dataset):
    ...
    Supports three layouts:
      1. annotations.json + images/ directory (split field optional)
      2. Class subdirectories: crops_dir/{fiber,film,fragment}/*.png
      3. Split subdirectories: crops_dir/{train,val}/{fiber,film,fragment}/*.png
    """
    ...
    def __init__(self, crops_dir: str, transform=None, split: str = None):
        ...
        # Layout 3: train/val subdirectories with class folders
        split_dir = self.crops_dir / split if split else None
        has_split_dirs = (split_dir is not None and split_dir.is_dir() and
                         any((split_dir / cn).is_dir() for cn in CLASS_NAMES))
...
def train(crops_dir: str, epochs: int = 50, batch_size: int = 16, lr: float = 0.0001,
          save_dir: str = 'experiments/efficientnet', resume: str = None, val_split: float = 0.2):
    ...
    # Detect dataset layout to determine split strategy
    crops_path = Path(crops_dir)
    ann_file = crops_path / 'annotations.json'
    
    # Layout 3: train/val subdirectories
    has_split_dirs = ((crops_path / 'train').is_dir() and
                      any((crops_path / 'train' / cn).is_dir() for cn in CLASS_NAMES))
    # Layout 1: annotations.json with split field
...
def main():
    ...
    parser.add_argument('--model', type=str, default='experiments/efficientnet/efficientnet_best.pth',
    ...
    parser.add_argument('--save-dir', type=str, default='experiments/efficientnet',
